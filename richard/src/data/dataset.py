...
class AugmentedAllDataset(Dataset):
    def __init__(self, image_dir:str, mask_dir:str, augment_mask_dir: str,
        ...
        self.image_dir = image_dir
        self.mask_dir = mask_dir
        self.augment_mask_dir = augment_mask_dir
        self.augment = augment
        self.normalize = normalize
        
        # Get and sort filenames
        images_list = sorted([f for f in os.listdir(image_dir) if os.path.isfile(os.path.join(image_dir, f))])
        augment_masks_list = sorted([f for f in os.listdir(augment_mask_dir) if os.path.isfile(os.path.join(augment_mask_dir, f))])
        masks_list = sorted([f for f in os.listdir(mask_dir) if os.path.isfile(os.path.join(mask_dir, f))])

        # --- Validation Step ---
        if len(images_list) != len(masks_list) or len(images_list) != len(augment_masks_list):
            raise ValueError(f"Mismatch in number of files: {len(images_list)} images, {len(masks_list)} masks")
        
        if not images_list: # Check if lists are empty
            raise ValueError(f"Warning: No image files found in {image_dir}")

        for img_file, mask_file, augment_mask_file in zip(images_list, masks_list, augment_masks_list):
            # Extract base filename part after the first underscore
            img_base = img_file.split('_', 1)[-1] if '_' in img_file else img_file
            mask_base = mask_file.split('_', 1)[-1] if '_' in mask_file else mask_file
            augment_mask_base = augment_mask_file.split('_', 1)[-1] if '_' in augment_mask_file else augment_mask_file
            
            # Compare the base filenames
            if img_base != mask_base or img_base != augment_mask_base:
                raise ValueError(f"Filename mismatch detected after sorting (based on name after first '_'): Image '{img_file}' vs Mask '{mask_file}' or Augment Mask '{augment_mask_file}'")
        # --- End Validation ---

        # Store the validated and sorted lists
        self.images = images_list
        self.masks = masks_list
        self.augment_masks = augment_masks_list
