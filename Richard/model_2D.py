...
class SpatialGate2D(nn.Module):
    def __init__(self):
        ...
        self.spatial = BasicConv2D(2, 1, kernel_size, stride=1, padding=(kernel_size-1) // 2, relu=False)
...
class DownTransition(nn.Module):
    def __init__(self, inChans, nConvs, elu, dropout=False):
        ...
        #self.cbam = CBAM2D(gate_channels=16)
...
class DigSep2D(nn.Module):
    def __init__(self, inChans, elu):
        super(DigSep2D, self).__init__()
    ...
    def forward(self, x):
        ...
        x16 = x.repeat(1, 16, 1, 1)  # repeat along the channel dimension
